...
def upgrade() -> None:
    ...
    op.create_index("ix_users_google_user_id", "users", ["google_user_id"], unique=True, schema=SCHEMA)
    ...
    op.create_index("ix_emails_gmail_message_id", "emails", ["gmail_message_id"], schema=SCHEMA)
    op.create_index("ix_emails_gmail_thread_id", "emails", ["gmail_thread_id"], schema=SCHEMA)

    op.create_table(
        "email_analysis",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("email_id", sa.Integer(), sa.ForeignKey(f"{SCHEMA}.emails.id", ondelete="CASCADE"), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_table("digests", schema=SCHEMA)
    ...
    op.drop_table("draft_replies", schema=SCHEMA)
    ...
    op.drop_table("email_analysis", schema=SCHEMA)

    op.drop_index("ix_emails_gmail_thread_id", table_name="emails", schema=SCHEMA)
    op.drop_index("ix_emails_gmail_message_id", table_name="emails", schema=SCHEMA)
    ...
    op.drop_table("emails", schema=SCHEMA)

    op.drop_index("ix_users_google_user_id", table_name="users", schema=SCHEMA)
